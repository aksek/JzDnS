...
jdns_protocol = Proto("JzDns", "JzDns protoocol")

function jdns_protocol.dissector(buffer, pinfo, tree)
    length = buffer:len()
    
    if length == 0 then return end
    
    pinfo.cols.protocol = jdns_protocol.name
    
    local subtree = tree:add(jdns_protocol, buffer(), "JzDnS Protocol Data")
    
    decodedJson, e = json.decode(buffer():string())
    subtree = subtree:add(buffer(), "Message type: " .. decodedJson.header.type .. "\nUser: " .. decodedJson.header.user .. "\nSize: " .. decodedJson.header.size .. "\nControl: " .. decodedJson.header.control)
end

local udp_encap_table = DissectorTable.get("udp.port")
udp_encap_table:add(5011, jdns_protocol)
